...
export default {
  ...
  setup(props,{emit}){
     ...
     const onEdit =async()=>{
        const {data} = await axios.put(`http://localhost:8000/api/instructions/${props.id}`,newInstructionData)
        emit("onLoading");
         isEditing.value = false;
     }
      const onDelete = async()=>{
        const {data} = await axios.delete(`http://localhost:8000/api/instructions/${props.id}`)
        emit("onLoading");
     }
